...
class RandomModel(Model):
    ...
    def __init__(self, num_agents=1, porObs = 0.2, probTrash =0.5, width=8, height=8, seed=42):
        ...
        # Create Random Agent at position (1,1) with charging station
        start_cell = self.grid[(1, 1)]

        # Creamos una estación de carga en la celda (1,1)
        station = ChargingStationAgent(self, cell=start_cell)
        station_pos = start_cell.coordinate

        # Creamos el robot en (1,1) y le pasamos la posición de su estación
        RandomAgent(
            self,
            cell=start_cell,
            energy=100,
        ...
        obstacle_cells = self.random.sample(self.grid.empties.cells, num_obstacle_cells)
